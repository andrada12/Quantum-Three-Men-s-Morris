...
% valid_pairs/3
% valid_pairs(+Color, +State, -Pairs)

good_pair(State, Color, (X, Y)) :-
	cell(X),cell(Y),
	X @< Y,
	not(member(classic(X, _), State)), not(member(classic(Y, _), State)),
	not(member(quantum(X, Y, Color), State)).

valid_pairs(State, Color, Pairs) :- findall(T, good_pair(State, Color, T), Pairs).
...
good_move(State, Color, move(X,quantum(POS1,POS2,Color))) :-
	good_pair(State,Color,(POS1, POS2)),
	member(X,State),
...
valid_moves(State,Color,Moves) :- findall(T, good_move(State,Color, T), Moves).
...
castiga_white(State) :- 
	member(classic(pos(1,1), white), State),
	member(classic(pos(1,2), white), State),
	member(classic(pos(1,3), white), State).
	
castiga_white(State) :- 
	 member(classic(pos(2,1), white), State),
	 member(classic(pos(2,2), white), State),
	 member(classic(pos(2,3), white), State).	

castiga_white(State) :- 
	 member(classic(pos(3,1), white), State),
	 member(classic(pos(3,2), white), State),
	 member(classic(pos(3,3), white), State).
...
castiga_black(State) :- 
	member(classic(pos(1,2), black), State),
	member(classic(pos(1,3), black), State),
	member(classic(pos(1,1), black), State).
	
castiga_black(State) :- 
	 member(classic(pos(2,1), black), State),
	 member(classic(pos(2,2), black), State),
	 member(classic(pos(2,3), black), State).	

castiga_black(State) :- 
	 member(classic(pos(3,1), black), State),
	 member(classic(pos(3,2), black), State),
	 member(classic(pos(3,3), black), State).
...
castiga_black(State) :- 
 ...
 winner(State, [white]) :- castiga_white(State), not(castiga_black(State)).
 winner(State, [black]) :- not(castiga_white(State)), castiga_black(State).
 winner(State, [white, black]) :- castiga_white(State), castiga_black(State).
